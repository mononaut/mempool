...
export class StateService {
  ...
  setNetworkBasedonUrl(url: string) {
    ...
    // horrible network regex:
    // /^\/                                         starts with a forward slash...
    // (?:[a-z]{2}(?:_[A-Z]{2}?)?                   optional locale prefix (non-capturing)
    // (?:preview\/)?                               optional "preview" prefix (non-capturing)
    // (bisq|testnet|liquidtestnet|liquid|signet)/  network string (captured as networkMatches[1])
    const networkMatches = url.match(/^\/(?:[a-z]{2}(?:_[A-Z]{2})?\/)?(?:preview\/)?(bisq|testnet|liquidtestnet|liquid|signet)/);
